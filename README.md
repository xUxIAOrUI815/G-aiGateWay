...
G-AIGateway 通过把LLM 调用转化为“IO 密集型的缓存检索”，在不侵入业务代码的情况下完成降本与提速。
...
- 
  - 启动方式
    1. 启动基础设施: Docker，可以使用DockerDesktop，确保项目容器是运行状态
    ```bash
    docker-compose up -d
    ```
    检查状态
    ...
    创建.env 文件并配置相关参数
    ```bash
    UPSTREAM_API_KEY=your_deepseek_api_key
    ...
    可以进行提问并从左侧面板观察缓存命中情况以及延迟情况
    ![alt text](115b5a664fa6c09ee104e1fedc383a0a.png)
